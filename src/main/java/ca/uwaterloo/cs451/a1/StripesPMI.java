...
public class StripesPMI extends Configured implements Tool {
    ...
    private static final class MyMapper2 extends Mapper<LongWritable, Text, Text, HashMapWritable<Text, PairOfFloatInt>> {
        private static final Text KEY = new Text();
        private static final Text PAIRWORD = new Text();
    ...
    private static final class MyReducer2 extends Reducer<Text, HashMapWritable<Text, PairOfFloatInt>, Text, HashMapWritable<Text, PairOfFloatInt>> {
        ...
        public void reduce(Text key, Iterable<HashMapWritable<Text, PairOfFloatInt>> values, Context context)
                throws IOException, InterruptedException {
            Iterator<HashMapWritable<Text, PairOfFloatInt>> iter = values.iterator();
            HashMapWritable<Text, PairOfFloatInt> map = new HashMapWritable<>();
            HashMapWritable<Text, PairOfFloatInt> map2 = new HashMapWritable<>();
            ...
            Text STAR = new Text("*");
            marginal = map.get(STAR).getRightElement();
            map.remove(STAR);

            for (Text term : map.keySet()) {
                PairOfFloatInt VALUE = new PairOfFloatInt();
                PairOfFloatInt pmi = map.get(term);

                VALUE.set((float) Math.log10(lineCount * pmi.getLeftElement() / marginal), (int)pmi.getRightElement());
                map2.put(term, VALUE);
            }
    ...
    private static final class MyPartitioner2 extends Partitioner<Text, HashMapWritable<Text, PairOfFloatInt>> {
        @Override
        public int getPartition(Text key, HashMapWritable<Text, PairOfFloatInt> value, int numReduceTasks) {
    ...
    public int run(String[] argv) throws Exception {
        ...
        long lineCount = job1.getCounters().findCounter(StripesPMI.LINENUM.counter).getValue();
